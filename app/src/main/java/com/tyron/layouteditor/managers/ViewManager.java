...
public class ViewManager implements BaseWidget.Manager {
	...
	public void updateAttributes(List<Attribute> attrs){
		...
		if(view.getTag(R.id.attributes) == null) {
			view.setTag(R.id.attributes, attrs);
		}else{
			LinkedHashSet<Attribute> attributes1 = new LinkedHashSet<>(attrs);
			attributes1.addAll((Collection<? extends Attribute>) view.getTag(R.id.attributes) );
			view.setTag(R.id.attributes, attributes1);
		}
